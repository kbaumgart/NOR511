...
const counties = require('../States/PAcounties.json');

module.exports = {
    PA:  function returnData(y) { //format a comma seperated return for insertion into the sql database of relevent datapoints
    return [ y.EventID, y.Facility, y.LaneStatus, y.Description, y.EventType, counties[y.County], y.IncidentMuniName, y.FromLat, y.FromLong, y.ToLat, y.ToLong, TimeCorrect.PA(y.DateTimeNotified), TimeCorrect.PA(y.CreateTime), TimeCorrect.PA(y.LastUpdate), y.DetourInEffect, TimeCorrect.PA(y.ActualDateTimeOpened), ""];},
